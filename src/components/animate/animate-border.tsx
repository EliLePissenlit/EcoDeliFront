...
export type AnimateBorderProps = BoxProps & {
  animate?: {
    ...
    repeatType?: 'loop' | 'reverse' | 'mirror'; // repeatType: ["loop", "reverse", "mirror"
...
export function AnimateBorder({ animate, sx }: AnimateBorderProps) {
  ...
  const values = {
    ...
    outline: animate?.outline ?? `135deg, ${theme.palette.grey[300]}, ${theme.palette.grey[300]}`,
  };
  ...
  const background = (color: string) => {
    ...
    const end = `transparent ${values.angle - (2 + values.length!)}deg, ${color}  ${
      values.angle
    }deg, transparent ${values.angle + values.length}deg`;

    return [
      `conic-gradient(from ${degs[0]}deg at ${values.distance! / aspectRatio}% ${
        values.distance
      }% , ${end})`,
      ...
      `conic-gradient(from ${degs[4]}deg at ${values.distance / aspectRatio}% ${
        values.distance
      }% , ${end})`,
    ];
  };
